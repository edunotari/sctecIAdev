...
from models import db, Empreendimento, SEGMENTOS_VALIDOS, STATUS_VALIDOS, CAMPOS_OPCIONAIS
...
def _validar_campos(dados, parcial=False):
    ...
    if "segmento" in dados and dados["segmento"] not in SEGMENTOS_VALIDOS:
        return f"Segmento inválido. Opções: {', '.join(sorted(SEGMENTOS_VALIDOS))}"

    if "status" in dados and dados["status"] not in STATUS_VALIDOS:
        return "Status inválido. Use 'ativo' ou 'inativo'."

    for campo_data in ("data_fundacao", "data_abertura"):
        if dados.get(campo_data):
            try:
                datetime.strptime(dados[campo_data], "%Y-%m-%d")
            except ValueError:
                return f"{campo_data} inválida. Use o formato YYYY-MM-DD."

    return None


def _aplicar_opcionais(emp, dados):
    """Aplica todos os campos opcionais do dict no objeto ORM."""
    for campo in CAMPOS_OPCIONAIS:
        if campo in dados:
            val = dados[campo]
            setattr(emp, campo, val.strip() if isinstance(val, str) else val or None)
...
def listar():
    ...
    if segmento:
        if segmento not in SEGMENTOS_VALIDOS:
            return _erro(f"Filtro 'segmento' inválido. Opções: {', '.join(sorted(SEGMENTOS_VALIDOS))}")
        query = query.filter_by(segmento=segmento)

    municipio = request.args.get("municipio")
    if municipio:
        query = query.filter(Empreendimento.municipio.ilike(f"%{municipio}%"))
    ...
    if q:
        ...
        query = query.filter(
            db.or_(
                Empreendimento.nome.ilike(t),
                Empreendimento.empreendedor.ilike(t),
                Empreendimento.municipio.ilike(t),
                Empreendimento.razao_social.ilike(t),
                Empreendimento.cnpj.ilike(t),
            )
        )
...
def atualizar(id):
    ...
    # Campos obrigatórios (quando presentes)
    for campo in ["nome", "empreendedor", "municipio", "contato"]:
        if campo in dados:
    ...
    if "data_abertura" in dados:
        emp.data_abertura = dados["data_abertura"] or None
